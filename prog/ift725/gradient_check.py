...
def evaluate_numerical_gradient(f, x, verbose=True, h=0.00001):
    """
    ...
    while not it.finished:
        ...
        ix = it.multi_index
        x_value = x[ix]
        x[ix] = x_value + h  # increment by h
        fxph = f(x)  # evalute f(x + h)
        x[ix] = x_value - h
        fxmh = f(x)  # evaluate f(x - h)
        x[ix] = x_value  # restore
...
def evaluate_numerical_gradient_blobs(f, inputs, output, h=1e-5):
    """
...
def eval_numerical_gradient_net(net, inputs, output, h=1e-5):
    return evaluate_numerical_gradient_blobs(lambda *args: net.forward(),
                                         inputs, output, h=h)


def check_gradient_sparse(f, x, analytic_grad, num_checks=10, h=1e-5):
    """
